...
#define SINE_TABLE_SIZE 255

const uint8_t sineLookupTable[SINE_TABLE_SIZE] = {
...
FadeInAndOut::FadeInAndOut(CRGB *leds, unsigned int ledCount) : Effect(leds, ledCount) {
}

void FadeInAndOut::run(unsigned long milliSeconds) {
    milliSeconds >>= 5;

    Serial.print(milliSeconds);
    Serial.print(' ');
    Serial.print(sizeof(sineLookupTable));
    Serial.print(' ');
    Serial.print(milliSeconds & SINE_TABLE_SIZE);
    Serial.print(' ');
    Serial.println(sineLookupTable[milliSeconds & SINE_TABLE_SIZE]);

    FastLED.setBrightness(sineLookupTable[milliSeconds % sizeof(sineLookupTable)]);
    FastLED.show();
}
